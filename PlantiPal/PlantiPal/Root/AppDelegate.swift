...
class AppDelegate: UIResponder, UIApplicationDelegate, CLLocationManagerDelegate {
        ...
        if(notifAccess == true) {
            if(gardenPlants.count != 0) {
                ...
                for plant in gardenPlants {
                    ...
                    if plant.isOutside == true {
                        if(isSnowy == true) {
                            scheduleWeatherNotif(plantName: plant.name)
                        }
                    }
                }
            }
    ...
    func scheduleWeatherNotif(plantName: String) {
        let content = UNMutableNotificationContent()
        content.title = "Weather Alert"
        content.body = "Today will snow. Cover " + plantName + " up to protect it!"
